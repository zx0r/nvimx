...
APP_NAME="nvimx"
APP_VERSION="0.1.0"
if [ -n "${NVIMX_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$NVIMX_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${NVIMX_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$NVIMX_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/zx0r/nvimx/releases/download/v0.1.0"
elif [ -n "${NVIMX_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$NVIMX_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/zx0r/nvimx/releases/download/v0.1.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/zx0r/nvimx/releases/download/v0.1.0"
fi
if [ -n "${NVIMX_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$NVIMX_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${NVIMX_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$NVIMX_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${NVIMX_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$NVIMX_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${NVIMX_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${NVIMX_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${NVIMX_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"nvimx","name":"nvimx","owner":"zx0r","release_type":"github"},"version":"0.1.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/nvimx"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/nvimx"
fi
...
The installer for nvimx 0.1.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/zx0r/nvimx/releases/download/v0.1.0
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set NVIMX_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "nvimx-aarch64-apple-darwin.tar.gz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="fa374e27799458eb3647c81de1232c28ac921cefb5bd7a8ebac5f9fb871f0bab"
            _bins="nvimx"
            _bins_js_array='"nvimx"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="nvimx-aarch64-apple-darwin-update"
            _updater_bin="nvimx-aarch64-apple-darwin-update"
            ;;
        "nvimx-aarch64-unknown-linux-gnu.tar.gz")
            _arch="aarch64-unknown-linux-gnu"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="0ce49971a3ca9f241491006fc1e4763bfff577c1f727bda2fa88e930c0d618e7"
            _bins="nvimx"
            _bins_js_array='"nvimx"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="nvimx-aarch64-unknown-linux-gnu-update"
            _updater_bin="nvimx-aarch64-unknown-linux-gnu-update"
            ;;
        "nvimx-aarch64-unknown-linux-musl.tar.gz")
            _arch="aarch64-unknown-linux-musl-static"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="355dea818cea27e81feb8a0729d0c15276a2f9f1df8e3f8f21b4cc5e07575ecd"
            _bins="nvimx"
            _bins_js_array='"nvimx"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="nvimx-aarch64-unknown-linux-musl-update"
            _updater_bin="nvimx-aarch64-unknown-linux-musl-update"
            ;;
        "nvimx-x86_64-apple-darwin.tar.gz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="a7f25702f9db785623161d24c5dc4adc0ce964b7584ac0a5cd2b540f7048f248"
            _bins="nvimx"
            _bins_js_array='"nvimx"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="nvimx-x86_64-apple-darwin-update"
            _updater_bin="nvimx-x86_64-apple-darwin-update"
            ;;
        "nvimx-x86_64-unknown-linux-gnu.tar.gz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="e74ff63ec5dba5ca6b770cea55351ad936dde9b5d61e910af869f02bb08a49db"
            _bins="nvimx"
            _bins_js_array='"nvimx"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="nvimx-x86_64-unknown-linux-gnu-update"
            _updater_bin="nvimx-x86_64-unknown-linux-gnu-update"
            ;;
        "nvimx-x86_64-unknown-linux-musl.tar.gz")
            _arch="x86_64-unknown-linux-musl-static"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="3d21fe5ca27faf5ccadc7adbd2f2da9fce6d60b9ee79d0cc9379579a191ccb9a"
            _bins="nvimx"
            _bins_js_array='"nvimx"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="nvimx-x86_64-unknown-linux-musl-update"
            _updater_bin="nvimx-x86_64-unknown-linux-musl-update"
            ;;
        *)
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="nvimx-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="nvimx-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="nvimx-aarch64-unknown-linux-gnu.tar.gz"
            ...
            _archive="nvimx-aarch64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="nvimx-x86_64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "39"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="nvimx-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${NVIMX_INSTALL_DIR:-}" ]; then
        _force_install_dir="$NVIMX_INSTALL_DIR"
    ...
    say "nvimx is installed! Run 'nvimx --help' to get started."
